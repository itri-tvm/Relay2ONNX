...
def infer_dtype(node):
    return infer_type(node).checked_type.dtype

def infer_shape(node):
    return infer_type(node).checked_type.shape
...
class MaxPool(ToOnnxOpConverter):
    ...
    name = 'MaxPool'
    @classmethod
    def convert(cls, ignores, graph, inputs, attrs):
        def cvtpadding2pads(padding):
            if len(padding) == 1:
                ...
                logging.error("The length of padding, %d, is tool long"%len(padding))
        transforms ={
            'dilation':'dilations',
            'pool_size': 'kernel_shape',
            'padding':('pads', [0,0,0,0], cvtpadding2pads)
        }
        extras ={'auto_pad':'NOTSET'}
        return AttrCvt(cls.name, transforms = transforms ,ignores = ignores ,extras=extras)(graph, inputs, attrs)

    @classmethod
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        assert attrs.setdefault('ceil_mode', 0) == False, 'Value of attr "ceil_mode" must be False.'
        ignores=['layout','ceil_mode']
        return cls.convert(ignores, graph, inputs, attrs)
    @classmethod
...
class ConvTranspose(ToOnnxOpConverter):
    ...
    def convert(cls, graph, inputs, attrs):
        ...
        extras={'output_shape': c.checked_type.concrete_shape(), 'auto_pad':'NOTSET'}
...
class Dropout(ToOnnxOpConverter):
    ...
    def _impl_v12(cls, graph, inputs, attrs={}, args=None):
        ...
        rate = graph.add_param('rate', rate_data, infer_dtype(args[0]))
...
class Reshape(ToOnnxOpConverter):
    ...
    def _impl_v5(cls, graph, inputs, attrs={}, args=None): 
        ...
        shape = graph.add_param('shape', shape_data)
...
class Pad(ToOnnxOpConverter):
    ...
    @classmethod
    def _impl_v11(cls, graph, inputs, attrs={}, args=None):
        assert attrs.setdefault('mode', 'constant') is 'constant', 'For opset=11, "pad_mode" must be "constant".'
        pads_data = cls._convert_pads(attrs['pad_width'])
        constant_value_data = attrs['pad_value']
        pads = graph.add_param('pads', pads_data)
        value = graph.add_param('constant_value', constant_value_data, infer_dtype(args[0]))
        ignores = ['pad_mode', 'pad_width', 'pad_value']
        return AttrCvt(cls.name, ignores=ignores)(graph, inputs+pads+value, attrs)

    @classmethod
    def _impl_v13(cls, graph, inputs, attrs={}, args=None):
        assert attrs.setdefault('mode', 'constant') is 'constant', 'For opset=13, "pad_mode" must be "constant".'
        pads_data = cls._convert_pads(attrs['pad_width'])
        constant_value_data = attrs['pad_value']
        pads = graph.add_param('pads', pads_data)
        value = graph.add_param('constant_value', constant_value_data, infer_dtype(args[0]))
...
class Tile(ToOnnxOpConverter):
    ...
    def _impl_v6(cls, graph, inputs, attrs={}, args=None):
        ...
        repeats = graph.add_param('repeats', repeats_data)
...
class Resize(ToOnnxOpConverter):
    ...
    @classmethod
    def _impl_v11(cls, graph, inputs, attrs={}, args=None):
        roi_data = np.array([],dtype='int64')
        scales_data = np.array([],dtype='float32')
        sizes_data = attrs['sizes']
        roi = graph.add_param('roi', roi_data)
        scales = graph.add_param('scales', scales_data)
        size = graph.add_param('sizes', sizes_data)
...
class Upsampling(ToOnnxOpConverter):
    ...
    def _impl_v9(cls, graph, inputs, attrs={}, args=None):
        ...
        scales = graph.add_param('scales', scales_data, infer_dtype(args[0]))
        return AttrCvt(cls.name,
            transforms={'method':('mode',None,cls._convert_mode)},
...
class StridedSlice(ToOnnxOpConverter):
    ...
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        ...
        return AttrCvt(cls.name,
                       ...
                       ignores=['strides', 'slice_mode'])(graph, inputs, attrs) 
    @classmethod
    def _impl_v10(cls, graph, inputs, attrs={}, args=None):
        ...
        starts = graph.add_param('starts', starts_data)
        ends = graph.add_param('ends', ends_data)
        axes = graph.add_param('axes', axes_data)
        steps = graph.add_param('steps', steps_data)
        inputs = inputs + starts + ends + axes + steps
        return AttrCvt(cls.name,ignores=['begin', 'end', 'strides', 'slice_mode'])(graph, inputs, attrs)
...
class OneHot(ToOnnxOpConverter):
    ...
    def _impl_v9(cls, graph, inputs, attrs={}, args=None):
        ...
        depth = graph.add_param('depth', depth_data)
...
class Full(ToOnnxOpConverter):
    ...
    def _impl_v9(cls, graph, inputs, attrs={}, args=None):
        ...
        shape = graph.add_param('input', attrs['shape'])
...
class Dense(ToOnnxOpConverter):
    ...
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        ...
        if isinstance(args[1], Const):
            value = args[1].data.asnumpy();
            value = np.transpose(value, [1,0])
            graph.del_last_const()
            b_t = graph.add_param('const', value, attrs['out_dtype'])
        else:
...
class BatchMatMul(ToOnnxOpConverter):
    ...
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        ...
        if isinstance(args[1], Const):
            ...
            b_t = ONNXNode('Transpose')(graph, [b_t], attrs={'perm':(0,2,1)})
...
class TopK(ToOnnxOpConverter):
    ...
    def _impl_v10(cls, graph, inputs, attrs={}, args=None):
        ...
        k = graph.add_param('k', attrs.get('k',1))
...
class Shape(ToOnnxOpConverter):
    """ Operator converter for shape_of op."""
    name = 'Shape'
    @classmethod
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        return AttrCvt(cls.name, ignores=['dtype'])(graph, inputs, attrs)
    @classmethod
    def _impl_v13(cls, graph, inputs, attrs={}, args=None):
        return AttrCvt(cls.name, ignores=['dtype'])(graph, inputs, attrs)

class Clip(ToOnnxOpConverter):    
    """ Operator converter for shape_of op."""
    name = 'Clip'
    @classmethod
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        return AttrCvt(cls.name, {'a_min': 'min', 'a_max': 'max'})(graph, inputs, attrs)
    @classmethod
    def _impl_v11(cls, graph, inputs, attrs={}, args=None):
        dtype = infer_dtype(args[0])
        min = graph.add_param('min', attrs['a_min'], dtype)
        max = graph.add_param('max', attrs['a_max'], dtype)
        return AttrCvt(cls.name)(graph, inputs + min + max)

class Cast(ToOnnxOpConverter):    
    """ Operator converter for cast op."""
    name = "Cast"
    @classmethod
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        return  AttrCvt(cls.name, {'dtype': 'to'})(graph, inputs, attrs)

    @classmethod
    def _impl_v6(cls, graph, inputs, attrs={}, args=None):
        def cvtdtypeto(dtype):
...
def _get_impl_map(opset):
    return {
        ...
        'shape_of':  Shape.get_impl(opset), # AttrCvt('Shape', ignores=['dtype']),
        'sign': ONNXNode('Sign'),
...
class ONNXGraph():
    ...
    def __init__(self, params, opset, graph_name, doc_string):
        ...
        self.ret_types = None
    def add_param(self, name, data, dtype=None):
        outputs = self.get_name(name)
        if dtype:
            node = numpy_helper.from_array(np.array(data, dtype),  outputs[0])
        else:
            node = numpy_helper.from_array(np.array(data),  outputs[0])
        self.initializer.append(node)
        return  outputs
    def del_last_const(self):
        if self.nodes[len(self.nodes)-1].op_type == "Constant":
    ...
    def add_const(self, const):
        ...
        return self.add_param('const',data)
