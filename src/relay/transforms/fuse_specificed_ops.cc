...
/*!
 *
 * \file src/relay/transforms/specific_fuse_ops.cc
 *
...
class GraphPartitioner {
 public:
  explicit GraphPartitioner(support::Arena* arena, Array<Array<ObjectRef>> op_attrs, int opt_level)
      : arena_(arena), op_attrs_(op_attrs), opt_level_(opt_level) {}
  /*!
   * \brief Group as a union find data structure.
   */
  struct Group {
	/*! \brief The trigger to fuse the group. */
 ...
 private:
  ...
  bool IsBiasAdd(const Object* node){
	    if(auto call_node = GetRef<ObjectRef>(node).as<CallNode>()){
	    	if(auto op = call_node->op.as<OpNode>()){
	    		if (op->name == "nn.bias_add"){
	    			return true;
	    		}
	    	}
	    }
  ...
  void CommitFuse_(IndexedForwardGraph::Node* src,
    ...
    if (!IsIncluded(sink)) return;
    if(IsBiasAdd(sink->ref) && src->pattern !=kOutEWiseFusable) return;
    visited_.insert(src);
    Group* gnode = groups_[src->index];
    CHECK(gnode != nullptr);
  ...
  bool IsIncluded(IndexedForwardGraph::Node* node){
	  if(auto call_node = GetRef<ObjectRef>(node->ref).as<CallNode>())
	  {
				...
				for (size_t i = 0; i < op_attrs_.size();i++)
				{
					if( op_attrs_[i].defined() && Downcast<String>(op_attrs_[i][0]) == op->name){
						if(!op_attrs_[i][1].defined()){
							...
							attrs_list.push_back(Downcast<Attrs>(op_attrs_[i][1]));
					}
				}
				for (Attrs attrs: attrs_list){
					LOG_IF(FATAL, op->attrs_type_index!=attrs->type_index())
							<< "The input attrs type is not consistant with the op name, \""
							<< op->name
							<< "\" .";
					if(AttrsComparor::CompareNonDefault(attrs, call_node->attrs)){
    ...
    for (size_t nid = 0; nid < groups_.size(); ++nid) {
      ...
      if (group_node->pattern == kOutEWiseFusable) {
        ...
        if (dom_node->parent != nullptr &&
          ...
          if (!IsBiasAdd(graph_node->ref) && CheckPath(graph_node, dom_node->parent->gnode, fcond)) {
            CommitFuse(graph_node, dom_node->parent->gnode);
          }
        }
...
class FuseSpecifiedOpsMutator : private ExprMutator {
 public:
	explicit FuseSpecifiedOpsMutator(int device_type):device_type_(device_type){}
  // Run the transform
  Expr Transform(const Expr& body, Array<Array<ObjectRef>> op_attrs,int fuse_opt_level) {
	// setup the group map.
    auto graph = IndexedForwardGraph::Create(&arena_, body);
    auto groups = GraphPartitioner(&arena_, op_attrs, fuse_opt_level).Partition(
        graph);
 ...
 private:
  ...
  Expr VisitExpr_(const CallNode* call) {
    if (call->op.as<OpNode>()) {
      ...
      if (ret_group->root_ref == call) {
        ...
        return MakeNewFunction(ret_group, device_type_, call->checked_type(), new_call);
      } else {
  ...
  Expr VisitExpr_(const TupleGetItemNode* tuple_get) {
    ...
    if (ret_group->root_ref == tuple_get) {
      ...
      return MakeNewFunction(ret_group, device_type_, tuple_get->checked_type(), new_node);
    }
  ...
  Array<Expr> GetNewArguments(const tvm::Array<Expr>& args,
    ...
    for (auto arg : args) {
      ...
      if (current_group->partition && current_group != arg_group) {
...
Expr FuseSpecifiedOps(const Expr& expr, Array<Array<ObjectRef>> op_attrs, int device_type, int fuse_opt_level, const IRModule& module) {
  return FuseSpecifiedOpsMutator(device_type).Transform(expr, op_attrs, fuse_opt_level);
}
} // namespace partition
namespace transform {

Pass FuseSpecifiedOps(Array<Array<ObjectRef>> op_attrs, int device_type, int fuse_opt_level) {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
    [=](Function f, IRModule m, PassContext pc) {
    int opt_level = fuse_opt_level == -1 ? pc->opt_level : fuse_opt_level;
    return Downcast<Function>(partition::FuseSpecifiedOps(f, op_attrs, device_type, opt_level,  m));
  };
  return CreateFunctionPass(pass_func, 1, "FuseSpecifiedOps", {"InferType"});
}

TVM_REGISTER_GLOBAL("relay._transform.FuseSpecifiedOps")
.set_body_typed(FuseSpecifiedOps);
